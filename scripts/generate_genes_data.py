#!/usr/bin/env python3
"""Generate ../data/genes_data.json for the Datalake Dashboard.

Extracts gene features from a GenomeDataLakeTables SQLite database and produces
a compact JSON array ready for the viewer's multi-track heatmap.

Each gene is a 36-element array (38 after phenotypes script appends 2 more).
Field indices are defined in config.json.

Usage:
    python3 generate_genes_data.py DB_PATH [OUTPUT_PATH]
"""
...
def parse_cluster_ids(raw):
    """Parse pangenome_cluster value, handling format with :size suffix."""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 generate_genes_data.py DB_PATH [OUTPUT_PATH]")
        sys.exit(1)

    db_path = sys.argv[1]
    output_path = sys.argv[2] if len(sys.argv) > 2 else "genes_data.json"
